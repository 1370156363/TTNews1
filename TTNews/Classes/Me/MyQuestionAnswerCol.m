...
@interface MyQuestionAnswerCol ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation MyQuestionAnswerCol
...
-(void)loadList
{
    ...
    [[KGNetworkManager sharedInstance] GetInvokeNetWorkAPIWith:KNetworkQuestionAnswerPush withUserInfo:dict success:^(id message) {
        ...
        if ([message[@"status"] integerValue] == 1) {
            NSMutableArray *newArr=[WendaModel mj_objectArrayWithKeyValuesArray:message[@"data"][@"beiyaoqing"]];
            if (newArr.count == 0) {
