...
@interface TiWenViewController ()<TZImagePickerControllerDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UIAlertViewDelegate,UINavigationControllerDelegate>
{
...
- (void)configCollectionView {
    ...
    _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 350, self.view.tz_width, self.view.tz_height - 350) collectionViewLayout:layout];
...
-(void)saveMessage
{
    ...
    if (_selectedPhotos.count > 0) {
        weakSelf(ws)
        [[KGNetworkManager sharedInstance] POST:@"" parameters:@"" uploadImages:_selectedPhotos uploadName:@"file[]" success:^(id message) {
...
-(void)upLoadDymanic:(NSString*)cover_id{
    NSMutableDictionary *prms=[@{
                                 @"uid":[[OWTool Instance] getUid], @"description":self.addMessageView.textContentView.text,
...
//    }
    ...
    [[KGNetworkManager sharedInstance] invokeNetWorkAPIWith:KNetworkWenAddWenDa withUserInfo:prms success:^(id message) {
...
-(AddMessageView *) addMessageView
{
    if (_addMessageView == nil) {
        _addMessageView = [[AddMessageView alloc]initWithState:YES];
